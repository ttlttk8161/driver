...
class PathPlanner:
    ...
    def initialize_center_line(self, waypoints_left: np.ndarray, waypoints_right: np.ndarray):
        ...
        """
        # 두 웨이포인트 배열의 길이를 맞춤
        min_len = min(len(waypoints_left), len(waypoints_right))
        waypoints_left = waypoints_left[:min_len]
        waypoints_right = waypoints_right[:min_len]
    ...
    def _find_closest_collision_distance(self, path: np.ndarray, obstacles: List) -> Optional[float]:
        ...
        for obstacle in obstacles:
            ...
            obs_pos = getattr(obstacle, 'position', [0, 0])
            # obs_radius = obstacle.get('radius', 1.0)
            obs_radius = getattr(obstacle, 'radius', 1.0)
    ...
    def _calculate_dynamic_safety_cost(self, path_candidate: Dict, obstacles: List) -> float:
        ...
        for obstacle in obstacles:
            ...
            if getattr(obstacle, 'type', None) != 'dynamic':
                continue
            
            # obstacle_pos = obstacle.get('position', np.array([0, 0]))
            obstacle_pos = getattr(obstacle, 'position', np.array([0, 0]))
            # obstacle_vel = obstacle.get('velocity', np.array([0, 0]))
            obstacle_vel = getattr(obstacle, 'velocity', np.array([0, 0]))
            # obstacle_size = obstacle.get('size', 0.5)
            obstacle_size = getattr(obstacle, 'size', 0.5)
