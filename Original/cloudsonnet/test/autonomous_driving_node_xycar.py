...
class AutonomousDrivingNode:
    ...
    def image_callback(self, msg):
        ...
        try:
            with self.image_lock:
                ...
                # Process image with lane detector
                lanes = self.lane_detector.detect_lanes(cv_image)
                
                # Update current position estimate based on lanes (simplified)
                if lanes:
                    self.update_position_from_lanes(lanes)
    ...
    def lidar_callback(self, msg):
        ...
        try:
            with self.lidar_lock:
                ...
                # Process lidar with obstacle detector
                obstacles = self.obstacle_detector.detect_obstacles(msg)
                self.detected_obstacles = obstacles
                
        except Exception as e:
            rospy.logerr(f"Lidar processing error: {e}")

    def update_position_from_lanes(self, lanes):
        """Update vehicle position estimate from lane detection (simplified)"""
        # This is a simplified position update - in real implementation
        # you would use proper localization methods
        current_time = time.time()
        dt = current_time - self.last_time
        
        # Simple dead reckoning update
        self.current_position[0] += self.current_velocity * np.cos(self.current_heading) * dt
        self.current_position[1] += self.current_velocity * np.sin(self.current_heading) * dt
        
        self.last_time = current_time
        
        # Create mock pose for compatibility
        class MockPose:
    ...
    def calculate_dynamic_safety_cost(self, path_candidate, obstacles):
        ...
        for obstacle in obstacles:
            for i, point in enumerate(path):
                ...
                collision_radius = 1.0 + obstacle.get('size', 0.5)  # Vehicle radius + obstacle
    ...
    def calculate_xycar_control_commands(self, optimal_path):
        """
        Calculate steering and speed commands for Xycar from optimal path
        """
        ...
        # Control gains
        K_angular = 30.0  # Higher gain for Xycar steering response
        ...
        # Apply limits for Xycar
        steering_angle = np.clip(steering_angle, -self.max_angle, self.max_angle)
        target_speed = np.clip(target_speed, 5, 20)  # Xycar speed range
        
        # Create Xycar motor message
        motor_msg = XycarMotor()
    ...
    def publish_path(self, path_candidate):
        ...
        if path_candidate is None or self.path_pub is None:
            return
        ...
        try:
            ...
            rospy.logwarn(f"Could not publish path: {e}")
    ...
    def control_loop(self, event):
        ...
        try:
            ...
            if self.current_pose is None:
                # Initialize pose if not set
                self.current_pose = type('MockPose', (), {
            ...
            if not path_candidates:
                rospy.logwarn("No path candidates generated - using emergency stop")
                motor_msg = XycarMotor()
                motor_msg.angle = 0
                motor_msg.speed = 0
            ...
            if optimal_path is None:
                rospy.logwarn("No optimal path found - using emergency stop")
                motor_msg = XycarMotor()
                motor_msg.angle = 0
                motor_msg.speed = 0
                self.motor_pub.publish(motor_msg)
            ...
            # Calculate Xycar control commands
            motor_msg = self.calculate_xycar_control_commands(optimal_path)
            ...
            rospy.logdebug(f"Xycar Control: angle={motor_msg.angle}, speed={motor_msg.speed}")
    ...
    def run(self):
        """Main run loop"""
        rospy.loginfo("Starting autonomous driving for Xycar...")
        rospy.spin()
