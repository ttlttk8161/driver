            ...
            while self._running:
                try:
                    ...
                    if selected_law_method:
                        ...
                        self.vehicle_interface.send_commands(steering=0.0, throttle=0.0, brake=0.2)

                    # --- [여기서부터 종합 summary 출력] ---
                    # planning 단계에서 전달받은 전체 결과 dict를 action_command와 함께 받는다고 가정
                    planning_results = getattr(action_command, 'planning_results', None)
                    if planning_results:
                        perception_info = planning_results.get('perception_info')
                        localization_info = planning_results.get('localization_info')
                        planned_path = planning_results.get('planned_path')
                        maneuver_decision = planning_results.get('maneuver_decision')
                    else:
                        perception_info = localization_info = planned_path = maneuver_decision = None

                    summary_lines = [
                        "\n================= [프로세스 단일 흐름 요약] =================",
                        f"[Perception]     {perception_info}",
                        f"[Localization]   {localization_info}",
                        f"[PlannedPath]    {planned_path}",
                        f"[Decision]       {maneuver_decision}",
                        f"[ActionCommand]  속도(m/s): {action_command.target_velocity_mps:.2f}  |  조향(rad): {action_command.target_steering_angle_rad:.3f}",
                        f"[ControlCmd]     스티어링(rad): {control_commands.steering_command:.3f}  |  Throttle: {control_commands.throttle_command:.2f}  |  Brake: {control_commands.brake_command:.2f}",
                        "========================================================\n"
                    ]
                    logger.info("\n".join(summary_lines))
                    # ---------------------------------------------------
